...
# Pygame käivitamine (graafika + heli moodulid)
pygame.init()
pygame.mixer.init()

# Fail, kuhu salvestatakse parim tulemus
HIGHSCORE_FILE = "highscore.txt"


def load_highscore():
    # Kontrollib, kas highscore fail eksisteerib
    if os.path.exists(HIGHSCORE_FILE):
        with open(HIGHSCORE_FILE, "r") as f:
            try:
                return int(f.read())  # loeb numbri failist
            except:
                return 0  # kui fail on katki, alustab nullist
    return 0


def save_highscore(score):
    # Salvestab uue parima tulemuse faili
    with open(HIGHSCORE_FILE, "w") as f:
        f.write(str(score))


# Laeb olemasoleva highscore
high_score = load_highscore()

# Taustamuusika seadistus ja käivitamine
pygame.mixer.music.load("Raining tacos.mp3")
pygame.mixer.music.set_volume(0.9)
pygame.mixer.music.play(-1)  # -1 = lõputu kordus

# --- PILDID ---
# Laetakse mängu graafika (õun, münt, ussi osad)
...
# Kõik ussi pildid samasse suurusesse
...
def your_score(score, high_score):
    # Näitab skoori ja highscore ekraanil
...
def get_direction(from_pos, to_pos):
    # Leiab, mis suunas liigub uss kahe punkti vahel
...
def gameloop():
    ...
    while not game_over:
        ...
        # hoiab ussi pikkuse kontrolli all
        if len(snake_list) > length_of_snake:
            del snake_list[0]

        # iseendaga kokkupõrge
